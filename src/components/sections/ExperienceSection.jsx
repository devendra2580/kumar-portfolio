...
const ExperienceSection = ({ setActiveTab }) => {
  ...
  return (
    <section id="experience" className="py-6 space-y-8 relative overflow-visible px-2 sm:px-4">
      ...
      <div className="relative space-y-8 relative z-10 overflow-visible">
        
        {/* Continuous Center-Aligned Vertical Line */}
        <div className="hidden md:block absolute left-[15px] top-6 bottom-8 w-0.5 bg-gradient-to-b from-blue-600 via-cyan-400 to-indigo-600 rounded-full z-0"></div>

        {experiences.map((exp) => (
          <div key={exp.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 sm:gap-4 items-start relative group overflow-visible">
            
            {/* Left Column: Timeline Icon Pin & Details */}
            <div className="md:col-span-2.5 sm:md:col-span-3 flex items-start space-x-3 relative pt-2 overflow-visible">
              ...
              <div className="space-y-0.5">
                <h4 className="text-xs sm:text-sm font-semibold text-blue-600 dark:text-blue-400">
                  {exp.period}
                </h4>
            ...
            <div className="md:col-span-9.5 sm:md:col-span-9 relative overflow-hidden bg-white/90 dark:bg-slate-800/90 backdrop-blur-md p-6 sm:p-8 rounded-3xl border border-slate-200/80 dark:border-slate-700/80 shadow-[0_4px_25px_rgba(0,0,0,0.03)] hover:-translate-y-1.5 hover:shadow-2xl hover:border-blue-400/50 transition-all duration-300 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 pb-2 border-b border-slate-100 dark:border-slate-700/60">
                
                <div className="flex items-center space-x-4">
                  {/* Company Logo Container */}
                  <div className="flex-shrink-0 p-2 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700/60 shadow-xs flex items-center justify-center">
                    {exp.logo}
                  </div>

                  {/* Role & Company Name */}
                  <div>
                    <h3 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white leading-snug">
                      {exp.role}
                    </h3>
                ...
                {/* Date & Location Badge (Top Right) */}
                <div className="flex flex-col sm:items-end space-y-1">
                  <span className="bg-blue-50 dark:bg-slate-700/70 text-blue-600 dark:text-blue-300 text-xs font-semibold px-3.5 py-1 rounded-full border border-blue-100 dark:border-slate-600/60 shadow-2xs">
                    📅 {exp.period}
                  </span>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {exp.skills.map((skill, sIdx) => (
                  <span
                    key={sIdx}
                    className="px-3 py-1 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-semibold border border-blue-200/50 dark:border-blue-500/30 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500 transition-colors duration-200 cursor-default"
                  >
                    #{skill}
                  </span>
                ))}
              ...
              <ul className="space-y-3 text-xs sm:text-sm text-gray-800 dark:text-gray-200 font-normal pt-1">
                {exp.points.map((pt, i) => (
                  <li key={i} className="flex items-start space-x-3 group/item">
                    <div className="w-5 h-5 rounded-full bg-blue-50 dark:bg-slate-700/80 text-blue-600 dark:text-blue-400 flex items-center justify-center flex-shrink-0 mt-0.5 shadow-2xs group-hover/item:scale-110 group-hover/item:bg-blue-600 group-hover/item:text-white transition-all duration-200">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-50 to-indigo-50/50 dark:from-slate-800/90 dark:to-slate-800/60 p-4 sm:p-5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 flex items-center space-x-3.5 shadow-xs">
